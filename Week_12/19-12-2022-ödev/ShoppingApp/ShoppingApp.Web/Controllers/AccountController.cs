...
namespace ShoppingApp.Web.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public IActionResult ConfirmLink()
        {
            return View();
        }

        [HttpPost]
        public async Task<IActionResult> ConfirmLink (string email)
        {
            if (String.IsNullOrEmpty(email))
            {
                TempData["Message"] = Jobs.CreateMessage("Hata", "Mail bilgilerini eksik girdiniz. Lütfen mailinizi eksiksiz girin.", "warning");
                return View();
            }
            var user=await _userManager.FindByEmailAsync(email);
            if (user == null)
            {
                TempData["Message"] = Jobs.CreateMessage("Hata", "Girdiğiniz maile ait bilgi bulunamadı.Tekrar deneyin.", "warning");
                return View();
            }
            var tokenCode = await _userManager.GenerateEmailConfirmationTokenAsync(user);
                var url = Url.Action("ConfirmDto", "Account",new
                {
                    userId=user.Id,
                    token=tokenCode
                });
            await _emailSender.SendEmailAsync(
                email,
                "ShoppingApp Mail Onaylama Linki",
                $"Mailinizi onaylamak için buraya <a href='http://localhost:7215{url}'>tıklayınız.</a>"
                );
            TempData["Message"]=Jobs.CreateMessage("Dikkat", "Onaylamanız,mail adresinize gönderilmiştir. Mailinizden onaylama linkine tıklayınız.", "info");
            return RedirectToAction("Login","Account");
        }
    }
}
