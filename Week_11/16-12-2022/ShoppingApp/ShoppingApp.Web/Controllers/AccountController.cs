    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginDto loginDto)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    TempData["Message"] = Jobs.CreateMessage("Bilgi", "Hesabınız onaylanmamış lütfen mailinize gelen nay linkine tıklayarak hesabınızı onaylayınız.", "warning");
                    return View(loginDto);
                    //Ödev: Eğer hesap onaylı değilse burada kullanıcıya "Onay Linki Gönder" şeklinde bir buton görünsün ve bu butona tıklandığında tekrar onay maili yollansın.
                }
        ...
        public async Task<IActionResult> Register(RegisterDto registerDto)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    TempData["Message"] = Jobs.CreateMessage("Bilgi", "Lütfen mail hesabınızı kontrol ediniz.Gelen linke tıklayarak hesabınızı onaylayın.", "warning");
        ...
        public async Task<IActionResult> ConfirmEmail(string userId, string token)
        {
            if (userId==null || token == null)
            {
                ...
                TempData["Message"] = Jobs.CreateMessage("Hata", "Geçersiz token yada users bilgisi", "danger");
                return View();
            }
        ...
        [HttpPost]
        public async Task<IActionResult> ForgotPassword(string email)//forgot passwordda name e email verdiğimiz için
        {
        ...
        public IActionResult ResetPassword(string userId, string token)
        {
            if (userId==null || token==null)
            {
                TempData["Message"] = Jobs.CreateMessage("Hata", "Bir sorun oluştu, lütfen daha sonra yeniden deneyiniz", "warning");
                return RedirectToAction("Index", "Action");
            }
            var resetPasswordModelDto = new ResetPasswordDto
            {
                Token = token
            };
            return View();
...
//account login edit view forgotpassword
